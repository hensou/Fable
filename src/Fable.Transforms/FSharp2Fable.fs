...
type private SignatureInfo = {|
    declaringMember: FSharpMemberOrFunctionOrValue option
...
let private getImplementedSignatureInfo com ctx r nonMangledNameConflicts
            (implementingEntity: FSharpEntity option) (sign: FSharpAbstractSignature) =
    ...
    | None -> {|
            declaringMember = None
            declaringEntity = None
            name = Naming.removeGetSetPrefix sign.Name
            isMangled = false
            isGetter = isGetter
            isSetter = isSetter
            hasSpread = false
        |}
    | Some(ent, _entFullName) ->
        let memb =
            ...
            if isMangled then
                let overloadHash =
                    if (isGetter || isSetter) && not indexedProp then ""
                        ...
                        |> Seq.mapToList (Seq.mapToList (fun p -> FsParam(p) :> Fable.Parameter))
        ...
        {|
            declaringMember = memb
            declaringEntity = Some ent
            name = name
            isMangled = isMangled
            isGetter = isGetter
            isSetter = isSetter
...
let private getMemberFunctionKind isGetter isSetter hasSpread args =
    match args with
    | _ when isGetter -> Fable.MemberGetter
    | arg::_ when isSetter -> Fable.MemberSetter(arg)
    | args -> Fable.MemberFunction(args, hasSpread)
...
let private transformObjExpr (com: IFableCompiler) (ctx: Context) (objType: FSharpType)
    ...
    let mapOverride (over: FSharpObjectExprOverride): Thunk<Fable.ObjectExprMember> =
      trampoline {
        ...
        let ctx, args = bindMemberArgs com ctx info.declaringMember over.CurriedParameterGroups
        let kind = getMemberFunctionKind info.isGetter info.isSetter info.hasSpread args
        let memberRef =
            match info.declaringEntity, info.declaringMember with
            | Some e, Some m -> Fable.MemberRef.Create(FsEnt.Ref(e), getMemberUniqueName m) |> Some
            | _ -> None
        let! body = transformExpr com ctx over.Body
        return { Name = info.name
                 Kind = kind
                 Body = body
                 GenericParams = over.GenericParameters |> Seq.mapToList (fun p -> FsGenParam(p))
                 MemberRef = memberRef
            }
      }
...
let private transformImplicitConstructor (com: FableCompiler) (ctx: Context)
    ...
    | Some ent ->
        ...
        let memberRef = Fable.MemberRef.Create(FsEnt.Ref(ent), getMemberUniqueName memb)
        let cons: Fable.MemberDecl =
            { Name = consName
              Kind = Fable.MemberFunction(args, hasParamArray memb)
              Body = body
              GenericParams = []
              UsedNames = set ctx.UsedNamesInDeclarationScope
              MemberRef = Some memberRef
...
let private transformImport _com r typ isPublic name kind selector path =
...
let private transformImportValue com r typ isMutable isPublic name selector path =
    if isMutable && isPublic then // See #1314
        "Imported members cannot be mutable and public, please make it private: " + name
        |> addError com [] None
    let kind = Fable.MemberValue(isMutable=false)
    transformImport com r typ isPublic name kind selector path
...
let private transformMemberValue (com: IFableCompiler) ctx isPublic name (memb: FSharpMemberOrFunctionOrValue) (value: FSharpExpr) =
    ...
    | Fable.Import(info, typ, r) when not info.IsCompilerGenerated ->
        ...
        transformImportValue com r typ memb.IsMutable isPublic name selector info.Path
    | fableValue ->
        ...
        let memberRef = memb.DeclaringEntity |> Option.map (fun e -> Fable.MemberRef.Create(FsEnt.Ref(e), getMemberUniqueName memb))
        [Fable.MemberDeclaration
            { Name = name
              IsPublic = isPublic
              Kind = Fable.MemberValue(memb.IsMutable)
              Body = fableValue
              MemberRef = memberRef
              GenericParams = []
              UsedNames = set ctx.UsedNamesInDeclarationScope
...
let private transformMemberFunction (com: IFableCompiler) ctx isPublic name (memb: FSharpMemberOrFunctionOrValue) args (body: FSharpExpr) =
    ...
    | Fable.Import(info, _, r) when not info.IsCompilerGenerated ->
        ...
        let kind =
            ...
            if memb.IsPropertyGetterMethod then Fable.MemberFunction([], hasSpread=false)
            else Fable.MemberValue(isMutable=false)
        transformImport com r typ isPublic name kind selector info.Path
    | body ->
        ...
        if memb.CompiledName = ".cctor" then
            ...
            let memberRef =
                ...
                |> Option.map (fun ent -> Fable.MemberRef.Create(FsEnt.Ref(ent), getMemberUniqueName memb))
            ...
            let body, kind =
                ...
                | JavaScript | TypeScript | Python ->
                    ...
                    | None -> body, Fable.MemberFunction(args, hasSpread)
                    | Some body -> body, Fable.MemberValue(isMutable=false)
                | _ -> body, Fable.MemberFunction(args, hasSpread)
            ...
            [Fable.MemberDeclaration
                { Name = name
                  ...
                  Kind = kind
                  Body = body
                  GenericParams = memb.GenericParameters |> Seq.mapToList (fun p -> FsGenParam(p))
                  UsedNames = set ctx.UsedNamesInDeclarationScope
                  MemberRef = memberRef
                  Tags = []
                  XmlDoc = tryGetXmlDoc memb.XmlDoc }]
...
let private transformMemberFunctionOrValue (com: IFableCompiler) ctx (memb: FSharpMemberOrFunctionOrValue) args (body: FSharpExpr) =
    ...
    | ImportAtt(selector, path) ->
        ...
        let typ = makeType Map.empty memb.FullType
        transformImportValue com None typ memb.IsMutable isPublic name selector path
    | _ ->
...
let private transformImplementedSignature (com: FableCompiler) (ctx: Context)
    ...
    let kind = getMemberFunctionKind info.isGetter info.isSetter info.hasSpread args
    let memberRef = Fable.MemberRef.Create(FsEnt.Ref(implementingEntity), getMemberUniqueName memb)
    let signatureRef =
        match info.declaringEntity, info.declaringMember with
        | Some e, Some m -> Fable.MemberRef.Create(FsEnt.Ref(e), getMemberUniqueName m) |> Some
        | _ -> None
    com.AddAttachedMember(entFullName, isMangled=info.isMangled, signatureRef=signatureRef, memb=
        { Name = info.name
          IsPublic = true
          Kind = kind
          Body = body
          MemberRef = Some memberRef
          GenericParams = signature.MethodGenericParameters |> Seq.mapToList (fun p -> FsGenParam(p))
          UsedNames = set ctx.UsedNamesInDeclarationScope
          Tags = []
          XmlDoc = tryGetXmlDoc memb.XmlDoc })
...
let private transformExplicitlyAttachedMember (com: FableCompiler) (ctx: Context)
    ...
    com.AddAttachedMember(entFullName, isMangled=false, signatureRef=None, memb =
        { Name = name
          Body = body
          IsPublic = isPublicMember memb
          Kind = getMemberFunctionKind isGetter isSetter hasSpread args
          GenericParams = memb.GenericParameters |> Seq.mapToList (fun p -> FsGenParam(p))
          UsedNames = set ctx.UsedNamesInDeclarationScope
          MemberRef = Fable.MemberRef.Create(FsEnt.Ref(declaringEntity), getMemberUniqueName memb) |> Some
          Tags = []
          XmlDoc = tryGetXmlDoc memb.XmlDoc })
...
let resolveInlineExpr (com: IFableCompiler) ctx info expr =
    ...
    | Fable.ObjectExpr(members, t, baseCall) ->
        let members = members |> List.map (fun m ->
            let kind =
                match m.Kind with
                | Fable.MemberFunction(args, hasSpread) ->
                    let args = args |> List.map (fun a -> { a with Ident = resolveInlineIdent ctx info a.Ident })
                    Fable.MemberFunction(args, hasSpread)
                | Fable.MemberSetter a ->
                    Fable.MemberSetter { a with Ident = resolveInlineIdent ctx info a.Ident }
                | Fable.MemberGetter
                | Fable.MemberValue _ as kind -> kind
            { m with Kind = kind
...
type private AttachedMembers = {|
    ...
    Members: ResizeArray<Fable.MemberDecl * Fable.MemberRef option>
...
type FableCompiler(com: Compiler) =
    ...
    member this.AddAttachedMember(entityFullName, isMangled, signatureRef, memb: Fable.MemberDecl) =
        this.ReplaceAttachedMembers(entityFullName, fun members ->
            if not isMangled then
                members.NonMangledNames.Add(memb.Name) |> ignore
            members.Members.Add((memb, signatureRef))
            members)
